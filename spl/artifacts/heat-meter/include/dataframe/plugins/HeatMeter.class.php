...
class HeatMeter extends Device {
  ...
  */
  ...
  public static function detect_order($data_frame) {
    for ($i = 0; $i < count($data_frame['heat_meters']); $i++)
      self::$default_order[] = array($i * 3 + 1, $i * 3 + 2, $i * 3 + 3);
  }
  ...
  protected function check_no($no) {
    if (!is_numeric($no) || is_float($no) || $no < 1 || $no > count(self::$default_order) * 3)
      return false;
    return $no;
  }
  ...
  public function fetch_by($data_frame) {
    if (!isset($data_frame['heat_meters']) || !isset($data_frame['heat_meters'][ceil(($this->no / 3) - 1)]))
      return null;
  ...
  public function render_box() {
    ...
    $unit = $data ? ($this->key == 'current_power' ? 'kW' : ($this->key == 'kwh' ? 'kWh' : 'MWh')) : '';
    echo <<<code

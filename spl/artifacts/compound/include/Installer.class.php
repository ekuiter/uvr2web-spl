...
code;
    ...
    $header
  	...
  	<p><a href="index.php">Back to $GLOBALS[brand]</a>.</p>
  	$footer
code;
      die($code);
    }
    ...
    <h2 style="margin:0;border-bottom:1px solid #aaa">$GLOBALS[brand] installation</h2>
code;
    if ($_POST || $_GET)
      $this->validate();
    else
...
code;
  ...
  private function form($server = 'localhost', $username = '', $database = '', $upload_password = '', $upload_interval = '10000', $data_record_interval = '90') {
    ...
    echo <<<code
    <p style="margin:20px 0;font-size:14px">To install $GLOBALS[brand], you need to fill out these fields.</p>
    ...
    <tr>
      ...
      <td style="font-size:14px">All settings, data frames and users of your old $GLOBALS[brand] installation are restored.</td>
    </tr>
    </tbody>
    </table>
    ...
    <tr>
      ...
      <td><input type="submit" value="Install $GLOBALS[brand]" style="width:150px;height:40px" /></td>
    </tr>
    </tbody>
    </table>
    </form>
code;
  ...
  private function form_filled_out() {
      $this->form(isset($_REQUEST['server']) ? $_REQUEST['server'] : '',
                  isset($_REQUEST['username']) ? $_REQUEST['username'] : '',
                  isset($_REQUEST['database']) ? $_REQUEST['database'] : '',
                  isset($_REQUEST['upload_password']) ? $_REQUEST['upload_password'] : '',
                  isset($_REQUEST['upload_interval']) ? $_REQUEST['upload_interval'] : '',
                  isset($_REQUEST['data_record_interval']) ? $_REQUEST['data_record_interval'] : '');
  }
  ...
  private function validate() {
    if (isset($_REQUEST['server']) && isset($_REQUEST['username']) && isset($_REQUEST['password']) && isset($_REQUEST['database'])
      && isset($_REQUEST['upload_password']) && isset($_REQUEST['upload_interval']) && isset($_REQUEST['data_record_interval'])) {
      try {
        DB::start($_REQUEST['server'], $_REQUEST['username'], $_REQUEST['password'], $_REQUEST['database']);
        DB::connect();
  ...
  private function install() {
    $config = array('active' => 'true',
      ...
      'upload_password' => $_REQUEST['upload_password'],
      'upload_interval' => (int) $_REQUEST['upload_interval'],
      'data_record_interval' => (int) $_REQUEST['data_record_interval']);
    Config::set_config($config);
    $this->create_tables();
    FrameCounter::set($config['data_record_interval'] - 1);
    if ($_FILES && $_FILES['backup']['tmp_name']) { 
      DB::multi_query(file_get_contents($_FILES['backup']['tmp_name']));
      $username = '?';
      $username_2 = '(use login data from the backup)';
      $password = '?';
    } else if (count(DB::query("SELECT * FROM uvr2web_users")) > 0) {
      $username = '?';
      $username_2 = '(use login data from existing installation)';
      $password = '?';
    } else {
      $username = 'admin';
      $username_2 = '';
      $password = $this->generate_password();
      DB::query("INSERT INTO uvr2web_users (username, password, role) VALUES('$username', '".DB::escape((new Password($password))->hash())."', 'admin')");
    }
    echo <<<code
